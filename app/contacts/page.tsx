...
export default function ContactsPage() {
  ...
  const [activeTab, setActiveTab] = useState<'contacts' | 'sheet'>('contacts')
    ...
    <div>
      ...
      <div className="flex gap-0 mb-4 border-b border-gray-200">
        <button onClick={() => setActiveTab('contacts')}
          ...
          <Network size={14} />人脈リスト
          <span className="text-[10px] bg-gray-100 text-gray-500 px-1.5 py-0.5 rounded-full ml-1">{contacts.length}</span>
        </button>
        <button onClick={() => setActiveTab('sheet')}
          className={clsx('px-4 py-2.5 text-sm font-medium border-b-2 -mb-px transition-colors flex items-center gap-1.5',
            activeTab === 'sheet' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700')}>
      ...
      {/* テーブル */}
      {activeTab === 'contacts' && <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="overflow-x-auto">
